//! `auditah license provision` — scaffold a license grid (`.toml`) in `LICENSES/`.
...
fn write_header(s: &mut String, id: &str) {
    writeln!(
        s,
        "# License definition generated by `auditah license provision`."
    )
    .unwrap();
    writeln!(s, "#").unwrap();
    writeln!(
...
/// # Errors
///
/// Returns `LicenseDefError` if the target file already exists or writing fails.
pub fn write_license_template(
    services: &Services,
    root: &Path,
    name: &str,
) -> Result<PathBuf, Report<LicenseDefError>> {
...
/// if the path has no file name. Used by `license provision --custom` to recover the id
...
pub fn write_grid(
    ...
    if path.exists() {
        return Err(Report::new(LicenseDefError)
    ...
    services
        .fs
        .write(&path, content)
        .change_context(LicenseDefError)
...
/// unknown/custom -> hard error pointing at `license provision --custom`.
///
...
pub(crate) fn provision_license(
    ...
    match well_known::resolve(license_id) {
        ResolveResult::NotFound => Err(Report::new(AppError)
            ...
            .attach("run `auditah license provision --custom <name>` to create it first")),
...
mod tests {
    ...
    fn provision_hard_errors_when_id_is_unknown_and_absent() {
        ...
        // Then it hard-errors with a pointer at `license provision --custom`.
