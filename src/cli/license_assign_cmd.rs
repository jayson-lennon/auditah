...
use crate::license_def::{provision_license, year_from_clock};
...
pub fn run(services: &Services, cmd: &LicenseAssignCmd) -> Result<CommandStatus, Report<AppError>> {
    ...
    if is_dir {
        ...
        println!(
            "license assign: wrote {}/{MANIFEST_FILENAME}",
            cmd.target.display()
        );
    } else {
        write_sidecar(services, &cmd.target, &record).change_context(AppError)?;
        println!("license assign: wrote {}.attr.toml", cmd.target.display());
    }
...
#[must_use]
pub fn resolve_start(cmd: &LicenseAssignCmd, is_dir: bool) -> PathBuf {
...
fn build_record(cmd: &LicenseAssignCmd, services: &Services, is_dir: bool) -> AttributionRecord {
    let title = cmd
        .title
        .clone()
...
mod tests {
    ...
    fn cmd(target: &str, id: &str, author: &str) -> LicenseAssignCmd {
        LicenseAssignCmd {
